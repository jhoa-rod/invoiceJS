...
import type { TaskImportPreviewRow } from "../types/import";
import { parseImportFile } from "../utils/fileParser";
import { commitTaskRows, normalizeTaskImportRows } from "../utils/importNormalizer";
...
interface ImportTasksProps {
  userId?: string;
  onImport: (tasks: Task[], statuses: string[]) => void;
}

export function ImportTasks({ userId, onImport }: ImportTasksProps) {
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const normalized = normalizeTaskImportRows(parsedRows, language);
  ...
  const updateRow = (rowId: string, field: keyof TaskImportPreviewRow, value: string | boolean) => {
    setRows((current) =>
  ...
  const handleConfirm = () => {
    const tasks = commitTaskRows(rows, userId);
    onImport(tasks, detectedStatuses);
    setRows([]);
    setDetectedStatuses([]);
    setFileName("");
  };
    ...
    <div className="grid gap-5">
      <div className="rounded-[24px] border border-dashed border-slate-200 bg-slate-50/70 p-5">
        <div className="flex flex-col gap-2">
          <h3 className="text-xl font-extrabold text-ink">{t("importTasks")}</h3>
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-500">
                <tr>
                  ...
                  <th className="px-4 py-3">{t("specifications")}</th>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-t border-slate-100 align-top">
                      ...
                      <td className="px-4 py-3">
                        <input
                          value={row.intercomLink}
                          onChange={(event) => updateRow(row.id, "intercomLink", event.target.value)}
                          className="w-44 rounded-xl border border-line px-3 py-2"
                        />
                        {row.intercomLink ? (
                          <button type="button" onClick={() => isValidUrl(row.intercomLink) ? openExternalUrl(row.intercomLink) : window.alert(t("invalidLink"))} className="mt-2 rounded-xl border border-line px-2 py-1 text-xs font-semibold text-ink">
                            {isIntercomUrl(row.intercomLink) ? t("openIntercom") : t("openChat")}
                          </button>
                        ) : null}
                      </td>
                      <td className="px-4 py-3">
                        <textarea
                          ...
                          className="min-h-20 w-52 rounded-xl border border-line px-3 py-2"
                        />
                      </td>
                      <td className="px-4 py-3">
                        <textarea
                          value={row.taskDetails}
                          onChange={(event) => updateRow(row.id, "taskDetails", event.target.value)}
                          className="min-h-20 w-52 rounded-xl border border-line px-3 py-2"
                        />
                      </td>
                      <td className="px-4 py-3">
                        <input
                          value={row.taskType}
                          onChange={(event) => updateRow(row.id, "taskType", event.target.value)}
                      ...
                      <td className="px-4 py-3">
                        <input
                          value={row.status}
                          onChange={(event) => updateRow(row.id, "status", normalizeText(event.target.value))}
                          className="w-36 rounded-xl border border-line px-3 py-2"
                        />
                      </td>
                      <td className="px-4 py-3">
                        <input
                          value={row.createdAt}
                          onChange={(event) => updateRow(row.id, "createdAt", event.target.value)}
                          className="w-36 rounded-xl border border-line px-3 py-2"
                        />
                      </td>
                      <td className="px-4 py-3">
                        <input
                          ...
                          onChange={(event) => updateRow(row.id, "completedAt", event.target.value)}
                          className="w-36 rounded-xl border border-line px-3 py-2"
                        />
                      </td>
                      <td className="px-4 py-3">
